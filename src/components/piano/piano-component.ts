...
import { KEY_LIST, OCTAVES_ARRAY } from './keys';
...
export class PianoUIComponent implements DOMGenerator, DOMEventSupplier, HTTPPrefetchSupplier {
    ...
    octaves_array: Key[][] = OCTAVES_ARRAY;
    ...
    async generateDOM(): Promise<void> {
        if (!this.htmlContainer) {
            this.htmlContainer = document.querySelector<HTMLDivElement>('#container')!;
        }
        ...
        this.htmlContainer.innerHTML = containerInnerHtml;
    ...
    async setOnlyOneOctaves(): Promise<void> {
        this.keyList = this.octaves_array[0];
        await this.generateDOM();
        await this.addDOMEvent();
    }
}
